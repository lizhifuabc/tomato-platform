	...
	private String grayVersion(List<GrayProperties.GrayServiceInfo.GrayVersionInfo> versions, int disturbanceHash) {
		...
		for (GrayProperties.GrayServiceInfo.GrayVersionInfo version : versions) {
			...
			if (disturbanceHash < sum) {
				...
				log.info("灰度配置|WeightGrayVersionStrategy|获取到的灰度版本:{}|disturbanceHash:{}", selectedVersion,
						disturbanceHash);
				return selectedVersion;
			}
		}

		log.warn("灰度配置|WeightGrayVersionStrategy|未获取到灰度版本:{}|weight:{}",
				GrayMetadataConstant.VersionStatus.UN_KNOW.getValue(), disturbanceHash);
