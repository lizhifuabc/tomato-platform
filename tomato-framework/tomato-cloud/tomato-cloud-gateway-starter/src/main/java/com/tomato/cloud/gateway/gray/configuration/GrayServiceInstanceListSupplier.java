...
public class GrayServiceInstanceListSupplier extends DelegatingServiceInstanceListSupplier {
	...
	public GrayServiceInstanceListSupplier(ServiceInstanceListSupplier delegate, GrayProperties grayProperties,
	...
	private String getGrayVersion(Request request) {
		...
		String version = grayVersionStrategy.getVersion(((RequestDataContext) request.getContext()).getClientRequest(),
				grayServiceInfo);
	...
	private List<ServiceInstance> filteredGrayVersion(List<ServiceInstance> instances, String grayVersion) {
		...
		if (GrayMetadataConstant.VersionStatus.UN_KNOW.getValue().equals(grayVersion)) {
			log.info("4灰度流程|GrayServiceInstanceListSupplier|filteredGrayVersion|实例:{}|版本号:{}|直接返回", getServiceId(),
					grayVersion);
			return instances;
		}
		// 通过灰度策略获取到版本号,过滤服务实例
		List<ServiceInstance> grayInstances = instances.stream().filter(i -> {
			String version = i.getMetadata()
				.getOrDefault(GrayMetadataConstant.VERSION, GrayMetadataConstant.VersionStatus.DEFAULT.getValue());
			return version.equals(grayVersion);
		}).collect(Collectors.toList());
		if (grayInstances.isEmpty()) {
			log.warn("灰度流程|GrayServiceInstanceListSupplier|filteredByHintVersion|实例:{}|版本号:{}|未获取到灰度服务实例",
					getServiceId(), grayVersion);
			return instances;
		}
		else {
			log.info("灰度流程|GrayServiceInstanceListSupplier|filteredByHintVersion|实例:{}|版本号:{}|获取到灰度服务实例:{}",
					getServiceId(), grayVersion, grayInstances);
