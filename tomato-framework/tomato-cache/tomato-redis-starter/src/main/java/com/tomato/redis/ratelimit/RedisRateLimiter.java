...
/**
 * redis 限流组件 replenishRate = burstCapacity ，可以实现稳定的速率。 burstCapacity >
 * replenishRate，允许临时突发。速率限制器需要在两次突发之间保留一段时间（根据replenishRate）， 因为两个连续的突发将导致请求丢弃（HTTP
 * 429-太多请求） See https://stripe.com/blog/rate-limiters and
...
public class RedisRateLimiter {
	...
	public Resp<RedisRateLimiterResp> isAllowed(@Valid RedisRateLimiterReq redisRateLimiterReq) {
		...
		List<Long> results = this.stringRedisTemplate.execute(this.script, keys, replenishRate + "", burstCapacity + "",
				"", requestedTokens + "");
		RedisRateLimiterResp redisRateLimiterResp = RedisRateLimiterResp.builder()
			.allowed(results.get(0) == 1L)
			.tokensLeft(results.get(1))
			.build();
		return Resp.of(redisRateLimiterResp);
	}
