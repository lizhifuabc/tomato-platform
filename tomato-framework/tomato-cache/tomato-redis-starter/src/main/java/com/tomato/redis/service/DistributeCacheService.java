...
public interface DistributeCacheService {
	 ...
	 * 2.缓存存在数据，直接返回
	 * <p>
	 * 3.缓存中存储的是 "" (空字符串)，直接返回 null
	 * <p>
	 * 4.从数据库查询数据,查询数据数据为空，缓存 "" (空字符串)并设置过期时间，返回 null
	 * <p>
	 * 5.从数据库查询数据,查询数据数据不为空，缓存数据并设置过期时间，返回数据
	 ...
	 * @param <ID> 查询数据库参数泛型，也是参数泛型类型
	 */
	<R, ID> List<R> getList(String keyPrefix, ID id, Class<R> type, Function<ID, List<R>> dbFunction, Long timeout,
			TimeUnit unit);
	 ...
	 */
	<R, ID> R getLogicalExpire(String keyPrefix, ID id, Class<R> type, Function<ID, R> dbFunction, Long timeout,
			TimeUnit unit);
	 ...
	 * @param <ID> 查询数据库泛型类型，也是参数泛型类型
	 */
	<R, ID> List<R> getLogicalExpireList(String keyPrefix, ID id, Class<R> type, Function<ID, List<R>> dbFunction,
	 ...
	 * @param <R> 结果泛型
	 */
	<R> List<R> getLogicalExpireList(String keyPrefix, Class<R> type, Supplier<List<R>> dbFunction, Long timeout,
			TimeUnit unit);
	 ...
	 * @param <ID> 查询数据库泛型类型，也是参数泛型类型
	 */
	<R, ID> R getWithLock(String keyPrefix, ID id, Class<R> type, Function<ID, R> dbFunction, Long timeout,
			TimeUnit unit);
	 ...
	 * @param <ID> 查询数据库泛型类型，也是参数泛型类型
	 */
	<R, ID> List<R> getWithLockList(String keyPrefix, ID id, Class<R> type, Function<ID, List<R>> dbFunction,
			Long timeout, TimeUnit unit);
	 ...
	 * @param <R> 结果泛型
	 */
	<R> List<R> getWithLockList(String keyPrefix, Class<R> type, Supplier<List<R>> dbFunction, Long timeout,
			TimeUnit unit);
