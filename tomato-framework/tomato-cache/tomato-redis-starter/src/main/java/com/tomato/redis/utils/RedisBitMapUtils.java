...
public class RedisBitMapUtils {
	...
	private long hash(String param) {
		...
		// murmur3_128() 是一个高效的 hash 算法，它的 hash 算法是将 key 拆分成多个块，每个块分别进行 hash 运算，最后将所有块的
		// hash 值进行合并，得到最终的 hash 值。
	 ...
	 * @param start
	 * 该参数的单位是byte（1byte=8bit），{@code setBit(key,7,true);}进行存储时，单位是bit。那么只需要统计[0,1]便可以统计到上述set的值。
	 ...
	 * <p>
	 * bitop and saveKey key [key...]，对一个或多个key逻辑并，结果保存到saveKey。 bitop or saveKey key
	 * [key...]，对一个或多个key逻辑或，结果保存到saveKey。 bitop xor saveKey key
	 * [key...]，对一个或多个key逻辑异或，结果保存到saveKey。 bitop xor saveKey
	 * key，对一个或多个key逻辑非，结果保存到saveKey。
	 * <p>
	 * @param op 元操作类型；
