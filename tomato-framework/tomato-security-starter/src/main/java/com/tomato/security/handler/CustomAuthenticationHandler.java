...
/**
 * 自定义认证处理器
 * <p>1.设置Content-Type为application/json;charset=UTF-8
 * <p>2.根据情况设置状态码
 * <p>3.将返回结果写入到response
 * <p>
 ...
 *  AccessDeniedHandler:权限不够处理器->认证通过，访问没有权限的 URI<br>
 *  AuthenticationEntryPoint:认证失败处理类->未登录访问，前端重定向到登录页面<br>
 * </p>
 *
 * @author lizhifu
 * @since 2023/4/23
...
public class CustomAuthenticationHandler implements AuthenticationSuccessHandler
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException authException) throws IOException, ServletException {
        ...
        response.getWriter().println(objectMapper.writeValueAsString(Resp.buildFailure(HttpStatus.UNAUTHORIZED.toString(), "认证异常")));
    }
    /**
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response, AccessDeniedException accessDeniedException) throws IOException, ServletException {
        ...
        log.warn("[handle][访问 URL({}) 时，用户({}) 权限不够:{}]", request.getRequestURI(), SecurityFrameworkUtil.getLoginUser(), detailMessage);
        response.setContentType(APPLICATION_JSON_UTF8_VALUE);
        response.setStatus(HttpStatus.FORBIDDEN.value());
        response.getWriter().println(objectMapper.writeValueAsString(Resp.buildFailure(HttpStatus.FORBIDDEN.toString(), "禁止访问")));
    }
    /**
     * 认证失败时的处理
     */
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        ...
        response.setContentType(APPLICATION_JSON_UTF8_VALUE);
        response.setStatus(HttpStatus.UNAUTHORIZED.value());
        response.getWriter().println(objectMapper.writeValueAsString(Resp.buildFailure(HttpStatus.UNAUTHORIZED.toString(), "登陆失败")));
    }
    /**
     * 认证成功时的处理,登陆成功
     */
    ...
    public void onExpiredSessionDetected(SessionInformationExpiredEvent event) throws IOException, ServletException {
        ...
        response.getWriter().println(objectMapper.writeValueAsString(Resp.buildFailure(HttpStatus.UNAUTHORIZED.toString(), message)));
    }
}
