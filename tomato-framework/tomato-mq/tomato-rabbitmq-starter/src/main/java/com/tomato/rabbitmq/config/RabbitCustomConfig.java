...
public class RabbitCustomConfig implements SmartInitializingSingleton {
    ...
    private void bindProducer(RabbitInfo rabbitInfo) {
        ...
        AbstractProducerService producerService = applicationContext.getBean(rabbitInfo.getProducer(), AbstractProducerService.class);
    ...
    private void bindConsumer(Queue queue, Exchange exchange, RabbitInfo rabbitInfo) throws Exception {
        ...
        CustomRetryListener customRetryListener = Optional.ofNullable(rabbitInfo.getRetry()).map(re-> applicationContext.getBean(re,CustomRetryListener.class)).orElse(null);
