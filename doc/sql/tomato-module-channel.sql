...
CREATE TABLE `t_pay_channel` (
    ...
    `processing_time`             DECIMAL(15, 2)  default null                  COMMENT '渠道平均交易处理时间',
    ...
    `daily_usage_time`            VARCHAR(100)    default null                  COMMENT '渠道单日可使用时间'
)engine = innodb auto_increment = 1  COMMENT '渠道表，用于存储不同支付渠道的信息';

# 路由规则表
drop table if exists `t_router_rule`;
CREATE TABLE `t_router_rule` (
    ...
    `status`                ENUM('ACTIVE', 'INACTIVE') NOT NULL                                     COMMENT '状态，active表示可用，inactive表示不可用',
    `filter_type`           ENUM('COST', 'RESPONSE_TIME', 'WEIGHT') default 'WEIGHT' NOT NULL       COMMENT '筛选方式，可以是成本、响应时间或权重',
    `version`         int             not null default 0                COMMENT '乐观锁',
    `update_time`     datetime        not null default current_timestamp on update current_timestamp comment '更新时间',
    `create_time`     datetime        not null default current_timestamp comment '创建时间'
)engine = innodb COMMENT '路由规则表，用于存储不同支付路由规则的信息';

# 路由与渠道绑定表
drop table if exists `t_router_channel_binding`;
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 COMMENT = '支付产品表';
...
insert into t_pay_product(`pay_product`,`pay_product_name`,`pay_product_desc`) values('9','信用支付','如京东的白条，蚂蚁花呗等，指使用信用账户进行透支，类似信用卡支付。');
