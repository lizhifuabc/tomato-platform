...
public class AccountManageService {
    ...
    public void cancelledAccount(AccountCancelledReq accountCancelledReq) {
        ...
        if (BigDecimal.ZERO.compareTo(account.getBalance()) != 0) {
            log.info("账号的账户余额不为零, merchantNo:{}", accountCancelledReq.getMerchantNo());
        ...
        //3.注销账户,账户管理序列号加1
        accountInfoManager.updateAccountStatus(account.getAccountNo(), AccountStatusEnum.ACCOUNT_CANCELLED.getValue() ,account.getVersion());
        // 4.创建账户管理记录
        AccountManageHisEntity accountManageHisEntity = new AccountManageHisEntity();
        accountManageHisEntity.setAccountNo(account.getAccountNo());
        ...
        accountManageHisEntity.setAfterValue("status:" + AccountStatusEnum.ACCOUNT_CANCELLED.getValue());
        accountManageHisDao.insert(accountManageHisEntity);
    }
