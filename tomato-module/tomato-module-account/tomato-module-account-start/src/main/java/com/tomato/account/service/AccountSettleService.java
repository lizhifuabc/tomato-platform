...
public class AccountSettleService {
	...
	public AccountSettleService(AccountSettleRecordManager accountSettleRecordManager, AccountInfoDao accountInfoDao,
			AccountSettleDao accountSettleDao, AccountSettleControlManager accountSettleControlManager) {
	...
	public LocalDate settle(String accountNo) {
		...
		// 查询账户结算控制
		AccountSettleControlEntity accountSettleControl = accountSettleControlManager.selectByAccountNo(accountNo)
			.orElseThrow(() -> new RuntimeException("账户结算控制不存在"));
		...
		// 结算信息
		AccountSettleEntity accountSettleEntity = accountSettleDao
			.selectByAccountNo(accountSettleControl.getAccountNo());
		// 创建结算记录
		AccountSettleRecordEntity accountSettleRecordEntity = accountSettleRecordManager.create(accountSettleControl,
				accountInfoEntity);
		// 更新结算控制：账户结算记录ID、下次结算日期
		LocalDate nextSettleDate = accountSettleControlManager.updateSettleControl(accountSettleEntity,
				accountSettleControl, accountSettleRecordEntity);
		...
		if (accountSettleRecordEntity.getSettleAmount().compareTo(BigDecimal.ZERO) > 0
			...
			accountTradDto.setAmount(
					accountSettleRecordEntity.getSettleAmount().add(accountSettleRecordEntity.getSettleFee()).negate());
