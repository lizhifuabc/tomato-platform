...
public class AccountOutReserveBalanceService {
	...
	public AccountOutReserveBalanceService(AccountSettleDao accountSettleDao, AccountHisDao accountHisDao,
			AccountOutReserveBalanceManager accountOutReserveBalanceManager, AccountInfoManager accountInfoManager) {
	...
	/**
	 * 计算账户风险预存期外余额 TODO 异步执行 TODO 分页 公式：风险预存期外余额 = 账户余额 - 风险预存期内的账户入账 + 风险预存期内的账户出账
	 * <p>
	 * 示例：当天时间：2023-04-20；风险预存期：1天；费率：0.1； 交易1：2023-04-20 11:11:11 100元 手续费：10元
	 * 交易2：2023-04-19 11:11:11 100元 手续费：10元 交易3：2023-04-18 11:11:11 100元 手续费：10元
	 * 交易4：2023-04-17 11:11:11 100元 手续费：10元 此时：余额：360；风险预存期外余额：360 - 90 = 270； 2023-04-20
	 * 提现 100元，手续费 40，余额：360 - （100+40）= 220；风险预存期外余额：270 - （100+40） = 130；
	 * </p>
	 * @param accountNo 账号
	 */
	...
	public void exe(String accountNo) {
		...
		log.info("计算账户风险预存期外余额：开始：账户：{}，执行时间：{}，当前时间：{}", accountNo, exeLocalDate, LocalDateTime.now());
		// 账户查询
		AccountInfoEntity accountInfoEntity = accountInfoManager.selectByAccountNo(accountNo)
		...
		log.info("计算账户风险预存期外余额：查询账户历史：账户：{}，开始日期：{}，结束日期：{}，风险预存期为：{}", accountNo, startDate, endDate,
				accountSettle.getReserveDays());
		// 风内：
		// 加款：交易、退款
		// 扣款：提现、结算
		...
		AccountHisCollectResBO collect = accountHisDao.collect(accountNo, startDate, endDate,
				AccountHisTypeEnum.TRAD.getValue());
		// 账户余额 - 账户历史金额
		...
		if (amount.compareTo(BigDecimal.ZERO) < 0) {
			log.error("计算账户风险预存期外余额：失败：账户：{}，开始日期：{}，结束日期：{}，风险预存期外余额小于0，查看是否存在未入账的账户历史", accountNo, startDate,
					endDate);
			return;
		}
		accountOutReserveBalanceManager.updateOutReserveBalance(accountInfoEntity.getAccountNo(), amount,
				accountInfoEntity.getVersion(), exeLocalDate);
