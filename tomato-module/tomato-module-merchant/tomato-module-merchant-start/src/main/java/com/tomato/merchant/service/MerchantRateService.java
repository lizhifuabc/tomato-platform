...
public class MerchantRateService {
    ...
    public MerchantRateService(MerchantRateRepository merchantRateRepository, MerchantInfoRepository merchantInfoRepository) {
    ...
    public MerchantRateDTO trade(String merchantNo, Integer payType){
        ...
        MerchantRate merchantRate = merchantRateRepository.findByMerchantNoAndPayType(merchantNo, payType).orElseThrow(()->new BusinessException("商户费率不存在"));
        // 判断交易费率状态
        ...
        // 将来交易费率生效时间已经生效
        Optional.ofNullable(merchantRate.getTradeRateEffectiveTime()).filter(effectiveTime->effectiveTime.isBefore(LocalDateTime.now())).ifPresent(effectiveTime->{
        ...
        // 将来分账费率生效时间已经生效
        Optional.ofNullable(merchantRate.getSplitRateEffectiveTime()).filter(effectiveTime->effectiveTime.isBefore(LocalDateTime.now())).ifPresent(effectiveTime->{
    ...
    public void rate(MerchantRateReq merchantRateReq){
        merchantInfoRepository.findByMerchantNo(merchantRateReq.getMerchantNo()).orElseThrow(()->new BusinessException("商户不存在"));
